...
def load_mnist_data(datadir):
    ...
    # model.eval()
    #
    # train_loader = torch.utils.data.DataLoader(dataset=mnist_train_ds,
    #                                            batch_size=60000,
    #                                            shuffle=False)
    # print(mnist_train_ds.target[:200])
    # print(mnist_train_ds.target.shape)
    # train_ans = []
    # for testImgs, labels in train_loader:
    #     testImgs = testImgs
    #     labels = labels
    #     outputs = model(testImgs)
    #     for element in outputs:
    #         tmp = element.detach().numpy()
    #         train_ans.append(np.argmax(tmp))
    #     break
    #     # print('-----------------------------')
    # train_target = torch.tensor(train_ans)
    # print(train_target[:200])
    # print(train_target.shape)
    #
    # test_loader = torch.utils.data.DataLoader(dataset=mnist_test_ds,
    #                                           batch_size=10000,
    #                                           shuffle=False)
    # # print(mnist_test_ds.target[:200])
    # # print(mnist_test_ds.target.shape)
    # test_ans = []
    ...
    #     for element in outputs:
    #         tmp = element.detach().numpy()
    #         test_ans.append(np.argmax(tmp))
    #     break
    #     # print('-----------------------------')
    # test_target = torch.tensor(test_ans)
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0, net_id=None, total=0, parties_id = None):
    if dataset in ('mnist', 'femnist', 'fmnist', 'cifar10', 'svhn', 'generated', 'covtype', 'a9a', 'rcv1', 'SUSY', 'cifar100', 'tinyimagenet'):
        ...
        if dataset == "tinyimagenet":
            ...
            # print("dataloader aaaaaaaaa")
            # train_ds = dl_obj(datadir, dataidxs=dataidxs, train=True, transform=transform_train, download=True, party_id=parties_id)
            # test_ds = dl_obj(datadir, train=False, transform=transform_test, download=True, party_id=parties_id)
        ...
        #         noisy_y = labels[random.randint(0, length-1)]
        #         train_ds[i] = {x, noisy_y}
