...
#         self.conv2 = nn.Conv2d(in_channels=3, out_channels=5, kernel_size=5, stride=1, padding=0)
#
#         self.fc1 = nn.Linear(in_features=5 * 5 * 5, out_features=15)
#         self.fc2 = nn.Linear(in_features=15, out_features=13)
#         self.fc3 = nn.Linear(in_features=13, out_features=10)  # number of classes/digits = 10
#
#     def forward(self, x):
#         # [64, 1, 28, 28]
...
class MNIST_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        mnist_dataobj = MNIST(self.root, self.train, transform, self.target_transform, self.download)
        ...
        #         probability = 0.1
        #     elif self.party_id == 3:
        #         probability = 0.1
        #     elif self.party_id == 4:
        #         probability = 0.1
        #     # if probability != 0:
        #     #     print("party_id: " + str(self.party_id))
        #     #     print("probability: " + str(probability))
        #     #     print("---------------------------")
        #     pre_target = target[i]
        #     if random.random() < probability:
        #         noisy_y = target_unique[random.randint(0, length - 1)]
        #         while noisy_y == pre_target:
        #             noisy_y = target_unique[random.randint(0, length - 1)]
        #         target[i] = noisy_y

        model = Net()
        model.load_state_dict(torch.load('./models/mnist_0430_1122.pth'))
        model.eval()

        loader = torch.utils.data.DataLoader(dataset=mnist_dataobj,
                                             batch_size=60000,
                                             shuffle=False)
        ans = []
        for testImgs, labels in loader:
            testImgs = testImgs
            labels = labels
...
class CIFAR10_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        target = torch.tensor(cifar_dataobj.targets)
